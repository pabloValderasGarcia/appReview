...
@section('content')
    <div class="container w-50" style="margin: 3em auto 15em auto;">
	    <div>
    		<h1 style="margin-bottom: 0.7em; font-size: 2.5rem; font-weight: bold">Edit review</h1>
    		<form action="{{ url('review/' . $review->id) }}" method="POST" enctype="multipart/form-data">
    		    ...
    		    <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <div class="d-flex align-items-center" style="height: 45px">
                        <div class="show-rate rate d-flex" style="all: initial; transform: rotateY(0deg); margin-left: auto; margin-right: 0.5em; font-size: 1.3em">
                            @for($i = 1; $i <= $review->rate; $i++)
                                <label class="starMarked">{{$i}} stars</label>
                            @endfor
                        ...
                        to
                        <div class="rate d-flex" style="font-size: 1.3em; margin-left: 0.5em">
    		    ...
    		    <div class="form-group mb-3">
    		        ...
    		        <input type="text" class="form-control" name="title" minlength="2" maxlength="40" value="{{ old('title', $review->title) }}" required/>
    		        @error('title')
                        <div class="alert alert-danger container">{{ $message }}</div>
    		        @enderror
    		    </div>
    		    ...
    		    <div class="form-group mb-3">
    		        ...
    		        <textarea type="text" class="form-control" style="height: 6em" name="excerpt" minlength="5" maxlength="250" required>{{ old('excerpt', $review->excerpt) }}</textarea>
    		        @error('excerpt')
                        <div class="alert alert-danger container">{{ $message }}</div>
    		        @enderror
    		    </div>
    		    ...
    		    <div class="form-group mb-3">
    		        ...
    		        <textarea id="messageReview" name="message" class="form-control" style="height: 23em">{{ old('message', $review->message) }}</textarea>
                    @error('message')
                        <div class="alert alert-danger container">{{ $message }}</div>
                    @enderror
    		    </div>
    		    ...
    		    <div class="form-group mb-3">
    		        ...
    		        <div class="upload__box">
                        <div class="upload__btn-box" style="position: relative">
                            ...
                            <input type="file" id="thumbnail_input" name="thumbnail" data-type="thumbnail" class="upload__thumbnail" style="color: transparent; background-color: transparent; position: absolute; bottom: 0; left: 35%; z-index: 0">
                        </div>
